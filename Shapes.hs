...
intersect (Ray e d) (Sphere p r mat)
  ...
  | t_2 < 0 = if t_1 < 0 then Nothing else let i = (e `add` (t_1 `Vecs.scale` d)) in Just (IR t_1 i (normalize (i `sub` p)) mat)
  | otherwise = let i = (e `add` (t_1 `Vecs.scale` d)) in Just (IR t_2 i (normalize (i `sub` p)) mat)
  where
